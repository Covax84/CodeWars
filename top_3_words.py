...
def top_3_words(text: str) -> list:
    """ Input:  string of text
        Output: an array of the top-3 most occurring words,
                in descending order of the number of occurrences
        Note: without regex, with a bit of hardcode.
    """
    counter = collections.Counter()
    cleaned_text = ''.join(x if x.isalpha() or x == "'" or x == ' ' else ' ' for x in text.lower())
    for word in cleaned_text.split():
        counter[word] += 1
    return [x[0] for x in counter.most_common(3) if x[0] != '' and x[0] != "'" and x[0] != "'''"]  # HARDCODE!
